...
RFID tqt_rfid;
 bool isCard() 
 {
	...
	status = MFRC522Request(PICC_REQIDL, str);	
        __LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "TQT In isCard: %d\n",status);
    if (status == MI_OK) {
		return true;
	} else { 
		return false; 
...
{
	uint8_t tqt_cfg_1 = (addr<<1)&0x7E;
        uint8_t tqt_cfg_2 = val;
	tqt_run_spi_master(&tqt_cfg_1);	
        __LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "W1: %d\n",tqt_cfg_1);
	tqt_run_spi_master(&tqt_cfg_2);
        __LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "W2: %d\n",tqt_cfg_2);
        //MFRC522_CS_HIGH;
}
...
void antennaOn(void)
{
	...
	if (!(temp & 0x03))
	{
                __LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "==TQT READ: %d=\n", temp);
...
{
	unsigned char val;
        uint8_t tqt_cfg_1 = ((addr<<1)&0x7E) | 0x80;
        uint8_t tqt_cfg_2 = 0x00;
	tqt_run_spi_master(&tqt_cfg_1);
        __LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "W3: %d\n",((addr<<1)&0x7E) | 0x80);
          val = tqt_run_spi_master(&tqt_cfg_2);
       __LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "R2: %d\n",val);
	return val;	
}
...
unsigned char MFRC522ToCard(unsigned char command, unsigned char *sendData, unsigned char sendLen, unsigned char *backData, unsigned int *backLen)
{
    ...
    //__LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "2: %d\n",(CommIEnReg<<1)&0x7E);
    ...
    setBitMask(FIFOLevelReg, 0x80);	
    //__LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "3: %d\n",(CommandReg<<1)&0x7E);
    ...
    if (i != 0)
    {    
    __LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "RUN\n");
...
unsigned char  MFRC522Request(unsigned char reqMode, unsigned char *TagType)
{
	...
	writeMFRC522(BitFramingReg, 0x07);		//TxLastBists = BitFramingReg[2..0]	???
        unsigned char tes = (BitFramingReg<<1)&0x7E;
        //__LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "1: %d\n",tes);
	TagType[0] = reqMode;
	status = MFRC522ToCard(PCD_TRANSCEIVE, TagType, 1, TagType, &backBits);
        __LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "STT: %d\n",status);
